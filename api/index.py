...
# Automatically initialize SQLite database tables and seed data on Vercel lambda cold start
try:
    call_command('migrate', interactive=False)
    from movies.models import Movie
    if not Movie.objects.exists():
        # Seed initial genres, languages, movies, theaters, and shows
        from movies.management.commands.seed_movies import Command as SeedCommand
        cmd = SeedCommand()
        # Fast seed for serverless container startup
        from movies.models import Genre, Language, Theater, Show
